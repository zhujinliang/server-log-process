...
all_scan_count=$(grep -i "/app/download" ${log_file} | cut -d ' ' -f 1 | sed '/,/d' | sort | uniq | wc -l)
...
all_download_count=$(grep -i ${ANDROID_APP_NAME} ${log_file} | cut -d ' ' -f 1 | sed '/,/d' | sort | uniq | wc -l)
